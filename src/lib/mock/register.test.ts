...
describe('mock/register — all 7 upstream paths registered', () => {
  it.each([
    ...
    '/v1/donation/categories',
  ])('list / dictionary endpoint %s', (path) => {
    expect(resolveMock(path)).toBeDefined()
  })

  it.each([
    '/v1/donation/charities/11111111-1111-4111-8111-000000000001',
    '/v1/donation/donation-projects/22222222-2222-4222-8222-000000000001',
    '/v1/donation/sale-items/33333333-3333-4333-8333-000000000001',
  ])('detail endpoint resolves the :id pattern (%s)', (path) => {
    expect(resolveMock(path)).toBeDefined()
  })

  it('list endpoint emits backend-shape payload (pageInfo + inflated categories)', () => {
    const handler = resolveMock('/v1/donation/charities')!
  ...
  it('categories dictionary returns 16 entries (matches CATEGORY_KEYS)', () => {
    const handler = resolveMock('/v1/donation/categories')!
