...
- **首頁 + 後台 placeholder（v0.9）** — `/` 顯示登入卡（帳號 / 密碼 + 「登入後台」+「建立帳號」）+ skip link 「demo」→ `/donation`；「登入後台」POST `/api/dev/login`（dev only，無需真實帳密驗證）→ `/cms`（v0.13 — 原 `/dashboard`，改名對齊 charity / project / sale-item CRUD 業務語意）；「建立帳號」→ `/register`（v0.12 — 原 `/admin`，因 BE `role=ADMIN` 命名衝突而搬家；spec 007 v0.2 已實作真實註冊 + auto-login 跳 `/cms`）；`/cms` 為 placeholder（`/admin` 留給未來真正 admin 後台），後台實際 CRUD 功能未開發
...
- **Global 5xx toast（v0.11）** — client fetch 拿到 5xx → 全局 toast「server 目前維修中…」（sonner）。同次操作多個請求並發 5xx 用 stable id (`'server-5xx'`) 自然 dedup，使用者只看到 1 個 banner；server 恢復後下個成功請求自動 dismiss。`<InlineError>` 維持區塊重試入口不替換。詳見 [spec 006](./specs/006-error-handling.md)
