...
- **依賴**：
  ...
  - cookie name `streamsight_session`（`env.SESSION_COOKIE_NAME`，預設值；vitest.config.ts `env` block 也固定相同值）
...
```
            ┌───────────────────┐
GET /cms ──▶│  src/proxy.ts     │── 無 streamsight_session cookie ──▶ 307 → /?reason=cms-auth
            │  (edge runtime)   │
            │  matcher: /cms*   │
            └────────┬──────────┘
                     │ 有 cookie
...
| 欄位 | 值 |
|---|---|
| 名稱 | `process.env.SESSION_COOKIE_NAME`（預設 `streamsight_session`） |
...
| # | 案例 | 期望 |
|---|---|---|
| 1 | `streamsight_session` cookie 存在 + 訪問 `/cms` | `proxy()` 不 redirect（pass through） |
| 2 | `streamsight_session` cookie 存在 + 訪問 `/cms/orders`（nested） | 同上 |
| 3 | 無 cookie + 訪問 `/cms` | 307 → `http://localhost:3000/?reason=cms-auth` |
| 4 | 有 `other=1` 但缺 `streamsight_session` + 訪問 `/cms/charities` | 307 → `/?reason=cms-auth` |
...
| 登入後手動清掉 `streamsight_session` cookie → 重新進 `/cms` → 跳 `/` + toast | proxy 層 |
| 把 `streamsight_session` 改成亂字串 → 進 `/cms` → 跳 `/` + toast | RSC 層（cookie 在但 unseal 失敗） |
