...
註冊成功 = backend 建 Account + PasswordCredential → BFF 拿到 tokens 後再打 `GET /auth/me` 取 user → 建 iron-session（與 [`/api/auth/login`](../../src/app/api/auth/login/route.ts) 同套）→ 帶 Set-Cookie 回 client → client `router.push('/cms')`。**不需要使用者再回首頁登入一次**。
...
Next.js 16 Server Actions 適合提交表單。本專案 LoginCard 走 `fetch('/api/auth/login')` 是因為要兼顧 client-side 載入狀態（`useTransition`）+ CSRF token 走 BFF route 比較直觀。Register 沿用同 pattern 一致性更高，也方便 unit test fetch mock。
...
X-CSRF-Token: <csrf>   # 走 csrf-check（非 /api/auth/login 的 csrfExempt 模式）
...

{
  "data": {
    ...
    "user": {
      ...
      "email": null,
      "displayOrder": null,
      "role": 0,                                // v0.3 — BE 008 demo policy：self-register 預設 ADMIN；FE decode access JWT role claim 取得（/me 不回 role）
      "createdAt": "<ISO>",
      "lastLoginAt": "<ISO>",
...
| `GET /auth/me` 回 `{ id, username, email, displayOrder, createdAt, updatedAt, lastLoginAt, lastLoginType }`（v0.3 — **不含 role**，role 只在 access JWT claims） | [BE 008 §6.4](../../../backend/docs/specs/008-auth-flow-password.md) / [BE 007 §10.10](../../../backend/docs/specs/007-auth-flow-google-oidc.md) | BFF 把 user 物件包進 iron-session；role 由 BFF decode access JWT 取得（同 `/api/auth/login` resolveRole pattern） |
...
- **`role` claim 在 FE 的用途**（v0.3 — 與 BE 008 demo policy 對齊）：BE 自助註冊（password 與 Google OAuth）一律建 `role: 0 = ADMIN`，access JWT 帶此 claim。FE BFF `/api/auth/register` `resolveRole(me.role, accessToken)` 從 JWT decode role（`/auth/me` 不回 role），寫入 session.role + ClientUser.role；註冊成功 push `/cms` 直接可用 admin 後台。Prod 應改 default USER + 顯式 promote flow
...
| 0.2.3 | 2026-06-16 | **註：v0.2.1 changelog 提到的「`/v1/donation/*` 才有版本前綴」已不再準確**（隨 [backend spec 023](../../../backend/docs/specs/023-api-routing-versioning.md) phase 2 cutover）：BE 把 donation 域 reads 從 `/v1/donation/*` 移到 `/user/v1/donation/*`，admin 從 `/v1/donation/*` 移到 `/cms/donation/*`；auth 仍維持無前綴（spec 023 §2.1 — auth 永遠等同 v1，不版本化）。本 spec 描述的 `/auth/register` / `/auth/me` BE call 仍正確，不需改動；v0.2.1 row 為 historical fix 紀錄保留原樣不修改。FE side donation 域 spec（002 / 004 系列 / 009 系列）已各自記錄 cutover。 |
| 0.3 | 2026-06-16 | **register 新帳號預設 ADMIN**（demo policy 配套 BE 008 / BE 007 改動）：(a) BE [`registerAccount`](../../../backend/src/lib/auth/service.ts) 與 Google OAuth register path 建 `prisma.account.create({ role: Role.ADMIN, ... })`，自助註冊一律 role=0；prod 應改 default USER + 顯式 promote flow；(b) FE BFF [`/api/auth/register/route.ts`](../../src/app/api/auth/register/route.ts) 引入 `resolveRole(me.role, accessToken)` helper（同 `/api/dev/login` v0.3 pattern；v0.5 起改名為 [`/api/auth/login`](../../src/app/api/auth/login/route.ts)）：`/me` 不回 role（[BE 008 §6.4](../../../backend/docs/specs/008-auth-flow-password.md)；spec 之前誤寫 `/me` 帶 role 是 drift），改 decode access JWT 的 `role` claim（[`src/lib/auth/decodeJwtPayload.ts`](../../src/lib/auth/decodeJwtPayload.ts)）；(c) §5.3 response sample `role: 0`、§6 BE assumption 表 `/me` shape 移除 role + 加註「role 從 JWT decode」、§7 register 段 role 一律 USER 的描述改寫為 ADMIN + decode JWT 為主來源；(d) `route.test.ts` BE_REGISTER_OK accessToken 改用 hand-rolled JWT 含 `role: 0` claim、BE_ME_OK 拿掉 role 欄、happy-path 斷言 `clientUser.role === 0`；其他 8 case 不變 |
| 0.4 | 2026-06-16 | **FE BFF 明確 stamp `role: 0` 給 BE 而非依賴 BE default**（隨 BE 008 改回 caller-supplied role 為主）：BE `registerAccount` 從 hardcode `role: Role.ADMIN` 改成 `role: input.role`（caller 給什麼用什麼、未帶 → DB default USER），route schema body 加 `role?: 0\|1` Type.Optional。FE BFF [`/api/auth/register/route.ts`](../../src/app/api/auth/register/route.ts) handler 改 `const upstreamBody = { ...body, role: Role.ADMIN }`，forward 給 BE 一律帶 `role: 0`。**為何在 BFF 而非 form 端**：(1) 防 form payload 被改、(2) future form 改動不致靜默 drop role、(3) demo policy 集中在 server-side。FE form RegisterCard 不需動。`route.test.ts` 加 `capturedRegisterBody` 捕捉 BE call body，斷言 `toMatchObject({ role: 0 })` |
| 0.5 | 2026-06-17 | **隨 spec 005 v0.7 — 對照端點 `/api/dev/login` → `/api/auth/login`**：本 spec 多處（§1 流程圖、§2.5、§6 BE 假設表、§5.1 contract 註解）將 dev-login link/text 改為 `/api/auth/login`。register 流程本身無變化（仍是 csrfExempt 的 POST → BE register + me → iron-session）。|
