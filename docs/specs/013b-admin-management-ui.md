...
狀態：**已實作（2026-07-18；導覽 2026-07-19 更新）**（v0.7；原 Draft v0.1，自 spec 013 v0.3 拆出）
關係：本檔為 [spec 013（索引）](./013-admin-management-page.md) 的**UI 半**。
契約 / 授權 / BFF / mutation 等業務邏輯見 [spec 013a](./013a-admin-management-logic.md)。

> **實作對齊（2026-07-18；導覽部分 2026-07-19 由 [016 v0.4](./016-cms-sidebar-streamlit-nav.md) 更新）**——已落地：
> `cms/layout.tsx` + 導覽（`CmsNav` → 016 v0.4 拆為 `CmsTopBar` 頂部系統切換 + `CmsSideNav` 左欄，super_admin 才顯示入口）、`/cms` landing
...
`cms/layout.tsx` + CMS 導覽（§1；`CmsNav` → 016 v0.4 拆為 `CmsTopBar` + `CmsSideNav`）、`ProfileForm`（可從 LoginCard 改）。

---

## 1. CMS 導覽（**本期待建的隱性交付項**）

> **⚠️ 已被 [spec 016 v0.4](./016-cms-sidebar-streamlit-nav.md) 取代（2026-07-19）**：本節描述的單一
> `CmsNav.tsx`（頂部橫向導覽）已**拆為兩層導覽**——`CmsTopBar.tsx`（頂部系統切換 + user/主題/登出）
> ＋ `CmsSideNav.tsx`（左欄：管理員管理 / 設定）。以下 §1 / §1.1 的 `CmsNav` 敘述請對照 016 v0.4：
> 入口可見性 gate 現於 `CmsSideNav`、登出按鈕現於 `CmsTopBar`。**super_admin gating 契約不變。**
...
「登出」按鈕，讓任何已認證 admin 一鍵登出 CMS（原置於 `CmsNav` 右側；016 v0.4 起移至
`CmsTopBar` 右側，ThemeToggle 旁）。
...
**測試：** 登出流程 4 tests（點擊綠燈 / MSW error / 非 2xx error / 按鈕存在）——原 `CmsNav.test.tsx`，
016 v0.4 起隨登出鈕移至 `CmsTopBar.test.tsx`。
...
```
src/app/cms/
├── layout.tsx               # CMS 外殼（RSC）；取 session 交給 TopBar/SideNav（§1；016 v0.4）
├── CmsTopBar.tsx            # client 頂部列：系統切換 + user/主題/登出（016 v0.4，取代 CmsNav）
├── CmsSideNav.tsx           # client 左欄：adminRole===super_admin 才顯示「管理員管理」（016 v0.4）
src/app/cms/admins/
...
**可後補（視覺類，e2e 兜底）**：純排版、徽章樣式、導覽（`CmsTopBar` / `CmsSideNav`）呈現。
...
最後更新：2026-07-19（v0.8，版面修訂：移除 admins 頁 sticky 標題列；「管理員管理＋人數」與「新增管理員」鈕合併一列，移至狀態 tab 之下、列表之上；EmptyState 文案「右上角」→「上方」。附帶解決收合側欄後展開鈕被 sticky 列遮蔽的問題（兩者同為 z-10、DOM 後者蓋前者，見 [016 §4.3](./016-cms-sidebar-streamlit-nav.md)）。純排版，§2.1 功能契約與測試不變。）
　　　　　　（v0.7，導覽對齊 [016 v0.4](./016-cms-sidebar-streamlit-nav.md)：`CmsNav` 拆為 `CmsTopBar`（頂部系統切換 + user/主題/登出）+ `CmsSideNav`（左欄 管理員管理/設定）；登出鈕與其 4 tests 移至 `CmsTopBar`；§1 加 supersede 註記、檔案結構更新。super_admin gating 契約不變。）
　　　　　　（v0.6，新增 §1.1 登出按鈕：`CmsNav.tsx` 右側登出按鈕 + CSRF 流程 + `CmsNav.test.tsx` 4 tests；551 tests pass。）
