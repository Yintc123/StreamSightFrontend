...
resource "aws_lb_target_group" "frontend" {
  ...
  # Liveness, not readiness: /api/health/live always returns 200 while the
  # server is up. /api/health returns 503 when Redis is briefly down — using
  # that here would cycle otherwise-healthy tasks.
  health_check {
